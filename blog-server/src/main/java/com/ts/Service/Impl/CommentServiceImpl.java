...
import com.ts.Model.VO.CommentVO;
...
public class CommentServiceImpl extends ServiceImpl<CommentMapper, Comment> implements ICommentService {
     ...
     *                    return: Result<String> error: 验证码错误,验证码过期,请刷新重试,? success:
     *                    null
     */
    @Override
    @RequestLog
    public Result addComment(CommentDTO commentDTO) {
        ...
        if (!RegexUtil.isHttps(comment.getUrl()))
            comment.setUrl(null);
        save(comment);
    ...
    public Result getCommentById(Long id) {
        ...
        // List<String> jsons=redisTemplate.opsForList().range(COMMENT_CACHE_KEY + id,
        // 0, -1);
        ...
        // redisTemplate.opsForList().rightPushAll(COMMENT_CACHE_KEY + id,
        // commentVOList.stream().map(JSONUtil::toJsonStr).toArray(String[]::new));
        // redisTemplate.expire(COMMENT_CACHE_KEY + id,
        // Duration.ofSeconds(COMMENT_CACHE_EXPIRE_TIME));
        // }
    ...
    public Result getCommentByPage(Long id, Integer current, Integer size) {
        ...
        Page<Comment> comments = page(page,
                new QueryWrapper<Comment>().eq("blog_id", id).eq("reply_id", -1).orderByDesc("create_time"));
